...
class FilmeMapperTest {
    ...
    void deveAtualizarEntidadeExistenteComDadosDoDTO() {
        FilmeDTO dtoAtualizado = FilmeDTO.builder()
                .titulo("Novo Título")
                .sinopse("Nova sinopse")
                .dataLancamento(LocalDate.of(2024, 5, 10))
                .ano(2023)
                .genero("Drama")
                .duracaoMinutos(150)
                .relevancia(new BigDecimal("8.50"))
        ...
        filmeMapper.atualizarEntidade(dtoAtualizado, filme);

        assertThat(filme.getId()).isEqualTo(1L);
        assertThat(filme.getTitulo()).isEqualTo("Novo Título");
        assertThat(filme.getSinopse()).isEqualTo("Nova sinopse");
        assertThat(filme.getDataLancamento()).isEqualTo(LocalDate.of(2024, 5, 10));
        assertThat(filme.getGenero()).isEqualTo("Drama");
        assertThat(filme.getDuracaoMinutos()).isEqualTo(150);
        assertThat(filme.getClassificacaoIndicativa()).isEqualTo("16 anos");
    }

    @Test
    @DisplayName("Não deve alterar entidade quando DTO for null")
    void naoDeveAlterarEntidadeQuandoDTOForNull() {
        filmeMapper.atualizarEntidade(null, filme);

        assertThat(filme.getId()).isEqualTo(1L);
        assertThat(filme.getTitulo()).isEqualTo("Filme de Teste");
        assertThat(filme.getGenero()).isEqualTo("Ação");
    }

    @Test
    @DisplayName("Não deve lançar erro quando entidade for null")
    void naoDeveLancarErroQuandoEntidadeForNull() {
        filmeMapper.atualizarEntidade(filmeDTO, null);
